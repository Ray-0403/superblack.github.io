...
def initialize_population(pop_size, time_steps, code_length):   #初始化种群，将所有个体的染色体进行二进制编码
    pop = []
    for _ in range(pop_size):                                   #pop_size=36(36个个体),code_length=6(染色体长度)
        indiv = []
        for _ in range(time_steps * code_length):
            indiv.append(1 if random.random() > 0.5 else 0)     #随机生成01编码
        pop.append(indiv)
    return pop


def decode(code):                  #对个体进行评价，计算各个编码出来的list的值。
    ...
    #int(,2)将字符串二进制编码转成整型二进制编码
    # math.pow(x,y)表示x的y次方
    # round(x,2)表示四舍五入后保留小数点后两位
    #其值的计算方式为转换为10进制，然后除以2的序列长度次方减一，也就是全一list的十进制减一。
    return element


def get_weight(indiv, time_steps, code_length):         #计算单个个体的适应度
    weight = []
...
def pop_to_weights(pop, time_steps, code_length):    #计算种群中的总体适应度
    weights = []
    for indiv in pop:
...
def initialize_weights(pop_size, time_steps, code_length):       #初始化新一代的种群
...
def crossover(indiv1, indiv2):   #基因重组
